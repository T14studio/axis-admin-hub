...
export default function LeadsPage() {
  ...
  async function fetchLeads() {
    setLoading(true);
    const { data } = await supabase.from("leads").select("id, name, email, phone, related_property_id, status, initial_message, created_at").order("created_at", { ascending: false });
    setLeads(data || []);
    setLoading(false);
  }
